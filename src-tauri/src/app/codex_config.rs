...
fn preflight_default_config(raw: &str) -> Result<(), AppError> {
    // Restoration is local. Codex resolves profiles, environment credentials,
    // keychain login and its default model; a billable probe cannot emulate it.
    let document = parse_document(raw)?;
    let provider_id = document.get("model_provider").and_then(Item::as_str).unwrap_or("openai");
    if let Some(base_url) = document.get("model_providers")
        .and_then(Item::as_table)
...
"#
    ...
    #[test]
    fn default_restore_accepts_codex_managed_defaults_and_credentials() {
        for raw in [
            "",
            ...
            preflight_default_config(raw).unwrap();
        }
        assert!(preflight_default_config("model = [").is_err());
        assert!(preflight_default_config("openai_base_url = \"http://example.test/v1\"").is_err());
    }
