...
def load_relay_config() -> tuple[str, str, str] | None:
    """Prefer the manager-managed independent image config over .env."""
    path = Path.home() / ".codex" / "imagegen-relay.json"
    try:
    ...
    model = str(config.get("model", "gpt-image-2")).strip() or "gpt-image-2"
...
def main() -> None:
    ...
    if relay_config:
        base_url, model, api_key = relay_config
    else:
