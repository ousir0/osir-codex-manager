    ...
    <section className="multi-model-prototype" aria-label="OpenCodex 多模型原型">
      ...
      <Sheet open={connectionMode !== null} onDismiss={() => setConnectionMode(null)} centeredInExpanded labelledBy="opencodex-connect-title" initialFocus="first">
        <div className="multi-model-modal">
          ...
          {connectionMode === "osir" ? <div className="multi-model-modal-body"><p>打开 OSIRAPI 后完成登录授权，再把一次性连接码粘贴到下方。连接码不会包含长期 API Key。</p><button className="btn ghost" type="button" onClick={() => void managerApi.openUrl("https://app.osirclaw.com")}><Icon name="external" />打开 OSIRAPI</button><label className="multi-model-modal-field"><span>一次性连接码</span><input className="input mono" value={connectionCode} onChange={(event) => setConnectionCode(event.target.value)} placeholder="osir://codex-manager/connect…" /></label></div> : null}
