...
本期落地“订阅用户通过浏览器 OAuth 登录并自动接入订阅套餐支持的平台模型”。当前已覆盖 GPT、Claude、Gemini、Grok；余额用户暂不进入本期实现，保留后续扩展接口和状态，不把余额计费逻辑混入当前 OAuth 主链路。
...
- OSIRAPI 根据订阅套餐实际包含的 active 接入分组，动态创建或复用对应平台的订阅专用 Key。
...
- 当前服务端会按订阅套餐实际平台创建或复用订阅专用 API Key。
...
- 订阅套餐 → 订阅配额池。
- 套餐包含的平台能力 → 对应平台订阅接入分组。
- API Key 绑定到平台订阅接入分组，不绑定到 quota pool 本身。
- 分组必须 active，且有至少一个可用模型。

当前接口先读取用户有效订阅，再读取套餐包含的访问分组，然后只为实际支持的平台创建或复用 Key。

### 5.3 平台开通条件

不再要求固定三个平台全部存在。对当前 OpenCodex 支持的平台，只有订阅套餐实际包含 active 接入分组且该分组存在可用模型时，才生成对应路由：

- 有有效的非系统订阅；
- 套餐包含对应平台的 active 接入分组；
- 对应分组至少有一个可用模型。

如果套餐没有任何可用平台，才拒绝接入并保持原有 Codex 配置不变。套餐减少或增加平台后，重新授权/兑换会重新读取当前分组，生成最新的 Key、Provider 和模型目录。

当前平台与 Provider ID 映射为：openai → osirapi-openai、anthropic → osirapi-claude、gemini → osirapi-gemini、grok → osirapi-grok。新增平台时只需扩展服务端平台注册表和对应协议能力，不修改 OAuth 主流程。
...
- 首次连接按套餐实际平台数量创建订阅 Key。
- 第二次连接按当前套餐复用或修复对应 Key，不产生重复 Key。
- 每把 Key 绑定对应平台的订阅接入分组。
...
- Codex 选择器能看到订阅接口实际返回的全部平台模型，包括 Gemini。
- 每个返回的平台至少完成一次最小请求验证。
- 路由失败时不自动切换到其他平台。
...
- 已修复服务端固定三平台校验：按订阅套餐 active 分组动态返回 1～4 个当前支持的平台。
- 已增加 Gemini 专用 Key 名称、Provider ID osirapi-gemini 和模型目录映射。
- 已修复 Manager Rust 接收逻辑和安装器，不再因 Provider 数量不是 3 而丢弃配置。
- 已补充动态平台、Gemini 和套餐减少平台的回归测试。

- OSIRAPI 生产基线候选提交：04f56776dc84deb0dcf11eae8877f30224113503（直接基于当前正式版本 b80c25a07）。
- Codex Manager 功能提交：e42cb7d。
- 两个提交均位于隔离功能分支，没有合并 main、没有打标签、没有触发正式发布。
- OSIRAPI 专项测试覆盖动态创建/复用 Key、加密配置下发和幂等重试。
